...
**更新日期**：2026-07-30（011 已关闭 + 012 已关闭）
...
| 核心架构 | 四维评分引擎（kline/fundamental/capital_flow/news）→ 5档评级（80/65/50/30 边界）→ 日报/看板/回测/自动优化/指数评级/智能预警(P3-B)/超买超卖徽标(003)/价格建议(005)/价格建议回测(007)/价格建议增强(009)/回测引擎方法论修复(010)/数据采集增量优化(011)/日志系统+诊断+健壮性(012) |
...
## 当前状态（2026-07-30，011+012 均已关闭）

### 已完成批次（B1~B27 + P3-B + 001~012 全部关闭）
...
| 表 | 记录数/最新日期 | 说明 |
|---|---|---|
| ratings_history | 230条，最新 2026-07-30（13天跨度） | 评级记录 |
...
| `data_collector.py` L1776/L1815/L1848 | 三处 `if False` 硬禁用，不可修改（012后行号） |
...
| `fetch_capital_flow` (L1577) | **011红线**：签名 `(symbol, market)` 不可加 force_full 参数，内部同日跳过逻辑不可修改 |
...
| `modules/data_collector.py` | **011增量+012错误增强版**（~2470行；fetch_kline/fetch_a_fundamental/fetch_hk_fundamental/fetch_north_capital/fetch_margin_balance/fetch_sentiment 均含 force_full；_log_error_to_db L287；collect_stock_data 统一入口；save_data_status 去重） |
| `modules/news_collector.py` | 消息面采集（011未改动，当日跳过在 fetch_sentiment 中实现） |
| `modules/daily_report.py` | **012改造版**（日报生成+进度追踪+超时控制+failure_summary；generate_daily_report；_update_progress_file；_process_single_stock；start_scheduler L85 每日18:00） |
| `config_weights.json` | 四维权重 + rating_mapping(80/65/50/30) + industry_overrides(7行业) |
| `config.py` | 011增量配置 + **012超时配置**：NORTH_CAPITAL_CACHE_DAYS=30 / FUNDAMENTAL_REPORT_TTL_DAYS=80 / PE_PB_CACHE_TTL_HOURS=24 / STOCK_TIMEOUT_SECONDS=90 / BATCH_TIMEOUT_SECONDS=1800 |
| `app.py` | Flask 主应用（**012-A**：main()全局FileHandler L3536-3561；011：/refresh-full API；含4处price_advice后处理+position_advice覆盖） |
| `templates/index.html` | 单页前端（含009网格表格+状态颜色编码+资金面/交易分析摘要） |
| `database/db_manager.py` | 建表（30张表，含010_ensure_price_backtest_columns + **012 error_logs ALTER TABLE migration**） |
| `logs/app.log` | **012新增**：全局文件日志（按日轮转，保留7天） |
| `logs/report_progress.json` | **012新增**：日报生成进度文件（实时状态） |
| `CHANGELOG.md` | 变更日志 |
...
| `docs/tasks/dev_tasks_20260730_011_hotfix.md` | **011 Hotfix开发任务书（时区Bug）** |
| `docs/tasks/qa_tasks_20260730_011_reverify.md` | **011 QA复验任务书** |
| `docs/tasks/dev_tasks_20260730_012_arch.md` | 012架构评审任务书 |
| `docs/tasks/dev_tasks_20260730_012_dev.md` | **012开发任务书** |
| `docs/tasks/qa_tasks_20260730_012.md` | **012 QA验收任务书** |
...
| `docs/reviews/review_012_logging_diagnosis_20260730.md` | **012架构师评审（457行，5决策点）** |
...
| `reports/qa_accept_011_incremental_20260730.md` | 011 QA首验（4P/3F，发现时区Bug） |
| `reports/dev_selftest_011_hotfix_20260730.md` | 011 Hotfix开发自验（V1-V4 PASS） |
| `reports/qa_reverify_011_hotfix_20260730.md` | **011 QA复验（R1-R4 全PASS）** |
| `reports/accept_011_incremental_20260730.md` | **011 PM+QA双签（已关闭）** |
| `reports/dev_selftest_012_logging_20260730.md` | **012开发自验（A/B/C全PASS）** |
| `reports/qa_accept_012_logging_20260730.md` | **012 QA验收（10/10 PASS）** |
| `reports/accept_012_logging_20260730.md` | **012 PM+QA双签（已关闭）** |

### 其他文档
| 文档 | 用途 |
|---|---|
| `docs/eval_backtest_guide_20260730.md` | **回测中心测评任务+查看指南** |
...
| **前端视觉验收** | 浏览器查看009网格表格+010可信样本+012日志效果 | 监理手动 |
...
| 011 /refresh-full API | POST /api/stocks/<id>/refresh-full（app.py） |
| 011 data_status去重 | save_data_status 先删后插，同维度同日仅1条 |
...
| 012 error_logs | +dimension(TEXT) +traceback(TEXT)；_log_error_to_db统一写入；traceback截断2000字符 |
| 012 failure_summary | generate_daily_report返回dict含failure_summary字段（fail_count>0时聚合by_reason） |
