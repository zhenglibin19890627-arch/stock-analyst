...
| 10 | `tests/` | 单元测试目录（pytest，MockDataProvider 隔离数据库与网络） | `conftest.py`, `test_scoring_engine.py` | `test_<模块名>.py` + `conftest.py`（固定名） |
| 11 | `__pycache__/` | Python 自动生成字节码缓存（**可安全删除**） | — | 自动生成 |
