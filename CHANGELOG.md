...
- 根因回顾：2026-08-17 实测 29 只股票全天停留在 09:56 早盘分数——收盘批次因 B11 复用（当日已有报告）全程跳过（耗时0s），收盘数据更新后未重算；用户"分析报告刷新→看板刷新"两步操作实为手动单只重算。
- 后端（`modules/daily_report.py` `_run_full_report_flow`）：交易日（weekday<5）收盘批次 `generate_daily_report(force=True)` 强制重算；非交易日保持默认复用（避免周末脏写与无效重算）。注意：force 重算伴随的采集受 011 增量门控保护，当日已有真实数据自动跳过，无重复网络开销。
- 前端（`static/js/app.js` `renderDailyReport`）：全部复用（新分析 0）时横幅改为 ⚠️ 醒目橙色样式，明示"沿用早前数据、未重新评分"，并引导勾选「强制全量刷新」或个股「🔄 刷新报告」。
- 测试：`tests/test_scheduler_force.py` 锁定调度语义——交易日 force=True / 非交易日 force=False（下游全 mock，不触网）。
- 验证：pytest 468 passed；node --check 通过；check_redlines 28/28；ruff 通过。注：021F 编号被并发批次占用，本批改用 021G。
